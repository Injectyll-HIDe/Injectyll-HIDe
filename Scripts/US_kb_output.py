...
if __name__ == '__main__':
    ...
    for line in Lines:
        try:
            ...
            line = line.strip()
            newArray = line.split(',')
            newArray.pop(0) #remove 'K' from packet indicating it's a keyboard packet

            #read in keyvalues, map them, add them to new array, then print entire array, then clear array
            for s in range(len(newArray)-1):
                if s == 0: #if modifier portion of array
                    if newArray[0] != "0":
                        intMod = int(newArray[0]) #get the integer value of the mod key
                        shift = getShift(int(newArray[0])) #check to see if it is a shift key
                        if shift:
                            mod = "Shift"
                        else:
                            mod = getMod(intMod) #translate the value of the mod key to a key press
                    else:
                        shift = False
                else: #keypress instead of modifier
                    if newArray[s] != "0": #if a value exists
                        if (shift==True):
                            if (exists(newArray[s],oldArray)==False): #if a new keypress is detected, add it
                                key = getUpperKey[newArray[s]]
                                pressArray.append(key)
                        else:
                            if (exists(newArray[s],oldArray)==False): #if a new keypress is detected, add it
                                key = getLowerKey[newArray[s]] #translate to upper case key
                                if (str(mod)==""):
                                   pressArray.append(str(key))
                                else:
                                    pressArray.append(str(mod)+ " && " + str(key))
            if len(pressArray) > 0:
                oF.write(str(pressArray)+"\r")
            oldArray = Cloning(newArray)
            newArray.clear()
            pressArray.clear()
            mod=""
